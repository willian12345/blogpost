...
Canvas 或者说 webgl 本身不像 DOM 那样每个元素都有自带的事件的系统

需要自定义实现事件系统，可交互的元素都应该是 DisplayObject 及继承自它的子类元素
...
要准确的响应用户点击的操作，必须先确定用户点击的范围在哪里，然后将范围内的 DisplayObject 显示元素触发对应用户绑定的点击事件回调
...
所以此处 target 的确定就是由 `this.hitTest(event.global.x, event.global.y) as FederatedEventTarget` 来确定的
...
可以看到当点击事件发生时，需要判断的不止是当前对象，而是当前 container 下的所有子对象，所以才需要用到  hitTestRecursive 即是递归判断
...
1. 进来先判断是否需要进行碰撞检测 `if (this._interactivePrune(currentTarget) || pruneFn(currentTarget, location))` 
   ...
   剔除水需要进行碰撞检测的对象，比如遮罩、不可见、不可交互、无需渲染等对象
...
我们终找知道我们 demo 中当没有指定 eventMode 为 static 或 dynamic 时，不会响应点击事件的原因了
...
this.worldTransform.applyInverse 方法，传入一个坐标点，返回一个新坐标点，这个新坐标点就是 sprite 本地坐标点
