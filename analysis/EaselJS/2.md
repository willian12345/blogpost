...
beginFill、 beginLinearGradientFill 等　Fill 操作

setStrokeStyle、setStrokeDash 等 Stroke 操作

这些操作 commit 均为 true 表明后面的绘制是新新的开始，将之前的一系列绘制命令归为一类

使用 `this._commitIndex` 游标重新指示命令数组内的位置



比如在 examples/Graphics_simple.html 内
...
可以理解为要开始新的 4 个不同的命令对象，存储在一维的 `_instructions` 数组命令内

当调用 setStrokeStyle、beginStroke、beginFill 等都会执行 `_updateInstructions` commit 命令归类
