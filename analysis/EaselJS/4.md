...
在 Stage 类的构造函数内 有这么一行 `this.enableDOMEvents(true);` 表示默认开启
...
`_handlePointerDown` 最重要的一句是  `target = o.target = this._getObjectsUnderPoint(o.x, o.y, null, true);`  判断点击位置是否存储显示对象
